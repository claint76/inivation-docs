...
static const size_t sizeFocalLengths = 6;
static const double focalLengths[sizeFocalLengths] = { 2.1, 2.8, 3.5, 4.5, 6.0, 12.0 };
...
int main(int argc, char **argv) {
	...
	for (size_t i = 0; i < sizeFocalLengths; i++) {
		...
		std::cout << focalLengths[i] <<  "mm lens -> Horizontal Linear FoV at 200cm (cm): " << lfov(width, focalLengths[i], 200.0) << std::endl;
		std::cout << focalLengths[i] <<  "mm lens -> Horizontal Linear FoV at 400cm (cm): " << lfov(width, focalLengths[i], 400.0) << std::endl;
	}
